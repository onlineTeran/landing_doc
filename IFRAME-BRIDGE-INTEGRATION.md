...
>
> **Статус: СПОСТЕРЕЖЕНО (з 2026-07-16).** Контракт відпрацьовано end-to-end на реальному стейджі
> продукту: продукт передає всі параметри за контрактом, лендінг авторозмірюється (~5300px),
> event_action-кнопки шлють дії батькові, локаль перемикається. Блокером запуску був ЛИШЕ origin
> whitelist — порядок запуску і діагностика: DEPLOY-AND-LAUNCH.md §6–7.
>
> Готове до копіювання ядро лежить у [`assets/iframe-bridge.js`](assets/iframe-bridge.js) — копіювати без змін.
> **Дослівний оригінал документа фронтенд-команди** (канонічне джерело, байт-в-байт, без редагувань):
> [`assets/IFRAME-BRIDGE-README.md`](assets/IFRAME-BRIDGE-README.md). Нижче — той самий контракт з м'якою
> нейтралізацією назви команди + наші адаптаційні розділи. При розбіжностях перевага — за оригіналом.
> Узагальнена модель у §11 головного документа лишається довідкою; **для реальної інтеграції використовуй цей контракт.**
>
> **Правило строгої відповідності (обов'язкове для кожного лендінга):** лендінг шле батькові ЛИШЕ типи
> з `T_IframeMessage` (`loaded`/`height`/`event_action`). Будь-який новий тип (напр. `analytics_event`)
> — спершу підтвердження команди продукту, до того — канал вимкнено прапорцем у коді. Параметри
> `hasAuth`/`locale`/`theme` в embedded читаються СТРОГО з `IframeBridge.config` (з його контрактними
> дефолтами `en`/`light`), НЕ з query вручну; власні дефолти лендінга дозволені лише в standalone,
> де контракт не діє.
