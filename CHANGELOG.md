...
### Changed
- SMARTICO §12 переписано за **вимірюванням на живому лейблі**, а не за
  документацією SDK: фільтрувати міні-ігри за прапорцями шаблона — пастка
  (`flow_builder_only` у 0 з 33, `only_in_custom_section` у 32 з 33 — такий
  фільтр лишає одну гру). Поле, яке відповідає «допущена до проду», —
  `custom_section_id`, звірений із `getCustomSections()`: шаблони посилаються на
  14 секцій, опублікованих 7, у сиротинських лежать зняті кампанії.
- Додано правило рендерити міні-ігри **на рівні секцій**: `menu_img` + `menu_name`,
  тап — `dp:gf_section&id=`. Це той самий рівень, який показує продукт.
- Порожній стан брати з `no_attempts_message` оператора, не вигадувати свій.
