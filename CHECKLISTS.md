...
- [ ] Точні URL для CTA (для iframe — перелік `event_action`-id від контент-менеджерів), дозволені origin-и асетів (CSP) та очікування щодо аналітики узгоджені з командою. **[O]**
...
- [ ] Кнопки — через єдиний `<CtaButton action-id>`; усі URL/`event_action`-id — лише в `config/actions.ts`, кожна дія має `analyticsId` (див. `CTA-AND-LINKS.md`). **[R]**
...
- [ ] Специфікацію аналітики (`GA-ANALYTICS-SPEC.md`) заповнено й передано продуктовим аналітикам; `cta_id` == `analyticsId` з `config/actions.ts`; події перевірені в GA4 DebugView (кожна — раз, без PII/token). **[R]**
